...
def _badge(parent, label, init, bg):
    wrap = tk.Frame(parent, bg="#1e293b")
    wrap.pack(side=tk.LEFT, padx=10)
...
mode_badge  = _badge(status_frame, "Mode",      "—",    "#64748b")
conf_badge  = _badge(status_frame, "Confidence","—",    "#475569")
gov_badge   = _badge(status_frame, "Governor",  "—",    "#475569")
...
# ── Button bar ────────────────────────────────────────────────────────────────
btn_bar = tk.Frame(root, bg="#dde3ea", pady=8)
btn_bar.pack(fill=tk.X, side=tk.BOTTOM)

def _btn(parent, text, cmd, bg, fg="white", width=13):
...
def _sep(parent, text):
    tk.Label(parent, text=text, bg="#dde3ea", fg="#94a3b8",
             font=("Helvetica", 8)).pack(side=tk.LEFT, padx=(12, 2))
...
def generate_report(records, session_dt, duration_s):
    ...
    with open(fpath, "w") as f:
        ...
        for mode, cnt in sorted(mode_counts.items()):
            ...
            f.write(f"  {mode:<22} {cnt:>5} samples  ({pct:.1f}%)\n")
        f.write("\n")

        # Governor distribution
        gov_counts = Counter(r.get("gov", "?") for r in records)
        f.write("GOVERNOR DISTRIBUTION\n")
        f.write("-" * 68 + "\n")
        for gov, cnt in sorted(gov_counts.items()):
            pct = 100.0 * cnt / len(records)
            f.write(f"  {gov:<22} {cnt:>5} samples  ({pct:.1f}%)\n")
        f.write("\n")
    ...
    csv_cols  = ["t_s","util","mhz","ipc","iowait","ctxsw",
                 "temp","power","mode","gov","auto"]
...
def _reset_graphs():
    global start_time
    timestamps.clear()
    for v in data_series.values(): v.clear()
    all_records.clear()
    start_time = None
...
def on_stop():
    ...
    if all_records and session_start_wall:
        ...
        recs, dt = list(all_records), session_start_wall
        threading.Thread(
...
# ── Buttons ───────────────────────────────────────────────────────────────────
_sep(btn_bar, "Monitor:")
_btn(btn_bar, "▶  Start", on_start, "#22c55e")
_btn(btn_bar, "■  Stop",  on_stop,  "#ef4444")

_sep(btn_bar, "Mode:")
_btn(btn_bar, "⟳ AUTO",          lambda: send_command({"cmd":"SET_MODE","mode":"AUTO"}),             "#3b82f6", width=9)
_btn(btn_bar, "💤 IDLE",          lambda: send_command({"cmd":"SET_MODE","mode":"IDLE"}),             "#64748b", width=9)
_btn(btn_bar, "🖱 INTERACTIVE",   lambda: send_command({"cmd":"SET_MODE","mode":"INTERACTIVE"}),      "#3b82f6", width=13)
_btn(btn_bar, "⚙ COMPUTE",       lambda: send_command({"cmd":"SET_MODE","mode":"COMPUTE"}),          "#f59e0b", width=11)
_btn(btn_bar, "⚡ LATENCY-CRIT", lambda: send_command({"cmd":"SET_MODE","mode":"LATENCY_CRITICAL"}), "#ef4444", width=14)
...
def update_gui():
    ...
    if is_monitoring:
        ...
        while not data_queue.empty():
            ...
            mode_badge.config(
                text=f"{mode}" if is_auto else f"{mode} 📌",
                bg=MODE_COLORS.get(mode, "#64748b"))

            conf_badge.config(
                text=f"{streak}/5 {'✓' if streak >= 5 else '…'}")

            gov_badge.config(
                text=gov,
                bg=GOV_COLORS.get(gov, "#475569"))
        ...
        if updated:
            for key, ln in lines_map.items():
                ...
                n    = min(len(timestamps), len(vals))
                if n > 0:
                    ln.set_data(timestamps[:n], vals[:n])
                    axes_map[key].relim()
